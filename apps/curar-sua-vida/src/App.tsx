...
export default function App() {
  ...
  const completedDays = reflections.filter(r => r.completed)
  const completedCount = completedDays.length
  const lastCompleted = completedDays.length > 0
    ? Math.max(...completedDays.map(r => r.day))
    : 0
  const currentDay = day ?? (lastCompleted === 0 ? 1 : Math.min(21, lastCompleted + 1))

  return (
    <div className="app">
      <header className="header">
        <h1>🪞 Curar a Sua Vida</h1>
        <span className="header-progress" aria-label={`${completedCount} de 21 dias concluídos`}>
          {completedCount}/21
        </span>
      </header>
      ...
      <nav className="bottom-nav" aria-label="Navegação principal">
        <button className={`nav-item ${route === 'home' ? 'active' : ''}`} onClick={() => navigate('home')} aria-label="Início">
          <span aria-hidden="true">🏠</span>Início
        </button>
        <button className={`nav-item ${route === 'journey' ? 'active' : ''}`} onClick={() => navigate('journey')} aria-label="Mapa dos 21 dias">
          <span aria-hidden="true">🗓</span>21 Dias
        </button>
        <button className={`nav-item ${route === 'professor' ? 'active' : ''}`} onClick={() => navigate('professor', currentDay)} aria-label="Professor IA">
          <span aria-hidden="true">🤖</span>Professor
        </button>
        <button className={`nav-item ${route === 'settings' ? 'active' : ''}`} onClick={() => navigate('settings')} aria-label="Ajustes">
